...
public class GAI_SingleTreeNode
{
    public GAI_MCTSParams params;

    private GAI_SingleTreeNode parent;
    private GAI_SingleTreeNode[] children;
    ...
    GAI_SingleTreeNode(GAI_MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private GAI_SingleTreeNode(GAI_MCTSParams p, GAI_SingleTreeNode parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new GAI_SingleTreeNode[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer, Types.TILETYPE[][] board) {
        ...
        while(!stop){
            ...
            GAI_SingleTreeNode selected = treePolicy(state);
            double delta = selected.rollOut(state, board);
    ...
    private GAI_SingleTreeNode treePolicy(GameState state) {

        GAI_SingleTreeNode cur = this;
    ...
    private GAI_SingleTreeNode expand(GameState state) {
        ...
        GAI_SingleTreeNode tn = new GAI_SingleTreeNode(params,this,bestAction,this.m_rnd,num_actions,
    ...
    private GAI_SingleTreeNode uct(GameState state) {
        GAI_SingleTreeNode selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (GAI_SingleTreeNode child : this.children)
        {
    ...
    private double rollOut(GameState state, Types.TILETYPE[][] board)
    {
        ...
        while (!finishRollout(state,thisDepth)) {
            int action = safeRandomAction(state, board);
    ...
    private void backUp(GAI_SingleTreeNode node, double result)
    {
        GAI_SingleTreeNode n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (GAI_SingleTreeNode tn : children) {
            if (tn == null) {
                return true;
            }
        }
